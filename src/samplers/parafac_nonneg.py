...
class NonnegParafacSampler(BaseSampler):
    def __init__(
        self,
    ...
    def _decompose_with_optional_mask(
        ...
        exp_tensor_eval = np.exp(standardized_tensor_eval)
        exp_tensor_eval[~tensor_eval_bool] = np.nan  # Keep unobserved values as NaN
        ...
        # Initialize the tensor with standard normal random values and exponentiate
        init_tensor_eval = np.exp(self.rng.normal(0, 1, tensor_eval.shape))

        # Assign observed values
        init_tensor_eval[tensor_eval_bool] = exp_tensor_eval[tensor_eval_bool]
        ...
        nn_tensor = cp_to_tensor(cp_tensor)

        # Apply log to revert to the original space
        log_tensor = np.log(nn_tensor + 1e-8)  # Add small epsilon to avoid log(0)
        log_tensor[tensor_eval_bool] = standardized_tensor_eval[tensor_eval_bool]
